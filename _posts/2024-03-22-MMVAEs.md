...
What would the process be for not only learning ohw to represent a horse (autoencoder), but to also represent a <b>distribution</b>
...
2. Calculate a sample: $$z = \mu(x) + \Sigma(x) \cdot \epsilon,\ where \ \epsilon \sim \mathcal{N}(0, 1)$$
...
$$ ln(p(x)) = ln \mathbb{E}[\frac{p(x|z)p(z)}{q(z)}] $$
